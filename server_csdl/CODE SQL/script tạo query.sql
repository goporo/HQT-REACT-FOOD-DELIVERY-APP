...
IF @LOAITK=0
	BEGIN
		SELECT QTV.MAQTV FROM QUANTRIVIEN QTV JOIN TAIKHOAN TK ON  QTV.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
	END
IF @LOAITK=1
BEGIN
		SELECT NV.MANHANVIEN FROM NHANVIEN NV JOIN TAIKHOAN TK ON  NV.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
END
...
IF @LOAITK=2
BEGIN
		SELECT DT.MADOITAC FROM DOITAC DT JOIN TAIKHOAN TK ON DT.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
END
IF @LOAITK=3
BEGIN	
		SELECT CN.MACN  FROM CHINHANH CN JOIN TAIKHOAN TK ON CN.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
END
IF @LOAITK=4
BEGIN
		...
		SELECT TX.MATX  FROM TAIXE TX JOIN TAIKHOAN TK ON TX.NGAYTG=TK.NGAYTG AND TX.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
END
...
IF @LOAITK=5
BEGIN
		SELECT KH.MAKH FROM KHACHHANG KH JOIN TAIKHOAN TK ON KH.NGAYTG=TK.NGAYTG AND KH.MATK=TK.MATK WHERE TK.TENTK=@TENTK AND TK.MATKHAUTK=@MATKHAUTK AND TK.TINHTRANGTK='AVAILABLE'
		COMMIT TRAN
		RETURN
END
GO	
...
AS
	if @THUTU='INCREASE'
	BEGIN
	SELECT TOP (@SL) * FROM THUCDON WHERE TINHTRANG='AVAILABLE' ORDER BY GIA ASC
	SELECT COUNT(TD.MAMONAN) SOLUONG FROM ( SELECT TOP (@SL) * FROM THUCDON WHERE TINHTRANG='AVAILABLE'  ORDER BY GIA ASC) TD
	RETURN
	END
	IF @THUTU='DECREASE'
	BEGIN
	SELECT TOP (@SL) * FROM THUCDON  WHERE TINHTRANG='AVAILABLE' ORDER BY GIA DESC
	SELECT COUNT(TD.MAMONAN) SOLUONG FROM ( SELECT TOP (@SL) * FROM THUCDON WHERE TINHTRANG='AVAILABLE' ORDER BY GIA DESC) TD
	RETURN
	END
GO
...
AS
	...
	SELECT COUNT(TD.MAMONAN) SOLUONG FROM ( SELECT TOP (@SL) * FROM THUCDON WHERE TINHTRANG='AVAILABLE' ORDER BY SLDH ASC) TD
	RETURN
	END
	IF @THUTU='DECREASE'
	BEGIN
	SELECT TOP (@SL) * FROM THUCDON ORDER BY GIA DESC
	SELECT COUNT(TD.MAMONAN) SOLUONG FROM ( SELECT TOP (@SL) * FROM THUCDON WHERE TINHTRANG='AVAILABLE' ORDER BY SLDH DESC) TD
	RETURN
	END
GO
...
AS
	...
	SELECT TOP (@SL) * FROM THUCDON WHERE MALAT=@MALAT  AND TINHTRANG='AVAILABLE' ORDER BY GIA
...
AS
	...
	ELSE IF @USER_TYPE = '4'
		BEGIN
		IF EXISTS(SELECT MADH FROM DON_DH WHERE MADH=@MADH AND (MATX=@MAKH OR MATX IS NULL))
		select tddh.MAMONAN, tddh.TENMONAN, tddh.GIABAN GIA, tddh.SOLUONG  from  THUCDONDATHANG tddh 
		WHERE  tddh.MADH = @MADH
		ELSE
...
AS
	...
	BEGIN
		...
		SET @TGBD =(SELECT CONVERT(DATE, NGAYTG) FROM TAIXE WHERE MATX=@MATX)
		SELECT TOP(100) MADH,TENTX,TRANGTHAIDH,PHISP FROM DON_DH WHERE NGAYLAP >= @TGBD AND MATX=@MATX
...
AS
	...
	WHERE  DH.NGAYLAP> DATEADD(DAY,-1,GETDATE()) AND DH.MATX IS NULL AND DH.TRANGTHAIDH='PROCESSING'
	ORDER BY NGAYLAP ASC
GO
...
BEGIN TRAN
	...
	BEGIN
		...
		SELECT @TENCN=CH.TENCH,@DIACHICN=CN.DIACHI,@PTHH=CN.PTHH,@MADT=CN.MADT FROM CUAHANG CH JOIN CHINHANH CN ON CN.MACN=CH.MACN WHERE CN.MACN=@MACN
		DECLARE @MADH INT
...
AS
	SELECT TOP (100) * FROM THONGTIN_HOPDONG TTHD JOIN HOPDONG HD ON TTHD.MAHD=HD.MAHD WHERE TTHD.MANHANVIEN IS NULL 
GO
...
EXEC sp_ThemPhamVi
EXEC sp_insert_LAT
EXEC sp_ThemHoaHong '01/01/2018','01/01/2020',10

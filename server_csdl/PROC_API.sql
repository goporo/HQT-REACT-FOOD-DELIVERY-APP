...
BEGIN TRAN
	...
	BEGIN
		...
		BEGIN
			...
			EXEC sp_ThemThucDonDatHang @MAMONAN,@TENMONAN,@MADH,@NGAYTAO,@GIA,@SOLUONG
...
BEGIN TRAN
	...
	BEGIN
		...
		BEGIN
			...
			SELECT @TENCN FROM CUAHANG WHERE MACN=@MACN
...
SELECT @TEMP=MATK FROM TAIKHOAN WHERE TENTK='DOITAC'
PRINT @TEMP
CREATE TYPE TEST AS TABLE (MA VARCHAR(10),TENTK VARCHAR(MAX))
GO
DECLARE @TEST TEST
INSERT INTO @TEST SELECT MATK,TENTK FROM TAIKHOAN
SELECT * FROM @TEST

...
BEGIN TRAN
	...
	BEGIN
		IF NOT EXISTS(SELECT MADH FROM DON_DH WITH(XLOCK,ROWLOCK) WHERE MADH=@MADH AND MACN=@MACN AND TRANGTHAIDH='AVAILABLE')
		BEGIN
			ROLLBACK TRAN
...
BEGIN TRAN
	...
	IF (@TRANGTHAIDH='CANCELED')
	BEGIN
		IF NOT EXISTS (SELECT MADH FROM DON_DH WITH(XLOCK,ROWLOCK) WHERE MADH=@MADH AND MAKH=@MAKH AND TRANGTHAIDH='AVAILABLE')
		BEGIN
			ROLLBACK TRAN
